...
module.exports = function generate(ctx) {
  ...
  let directories = [
    ctx.outputFolder,
    ctx.outputFolder + '/actions',
    ctx.outputFolder + '/effects',
    ctx.outputFolder + '/guards',
    ctx.outputFolder + '/models',
    ctx.outputFolder + '/reducers',
    ctx.outputFolder + '/resolvers',
  ...
  **/
  ...
  if (ctx.isNgrx === 'enabled') {
    schema = schema.concat([
      ...
      {
        template: './shared/effects/index.ejs',
        output: '/effects/index.ts',
        params: { models: ctx.models }
      },
      /**
       * NGRX Guards
  ...
  Object.keys(ctx.models).forEach(modelName => {
    if (ctx.models[modelName].sharedClass.ctor.settings.sdk &&
      ...
      */
      ...
      /**
      * SDK NGRX ACTIONS AND EFFECTS
      */
      if (ctx.isNgrx === 'enabled') {
        schema.push({
          template: './shared/actions/model.ejs',
          output: '/actions/' + modelName + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
